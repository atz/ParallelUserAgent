...
sub implementor
{
    ...
    unless (@{"${ic}::ISA"}) { # fixed in LWP 5.48
        ...
        eval "require $ic";  # try to autoload it
        if ($@) {
...
sub receive {
    ...
    LWP::Debug::trace("( [self]" .
                ...
                ", ". (defined $content ?
                    (ref($content) eq 'SCALAR' ? length($$content) . " bytes"  : '[ref('. ref($content) .')')
                    : '[undef]'
                ) .
                ", ". (defined $entry ? $entry : '[undef]') . ")");

    my($parse_head, $max_size, $parallel) = @{$self}{qw(parse_head max_size parallel)};
    ...
    # connections.  All the data we need is already in \$content, so we
    ...
    if (!defined($arg) || !$response->is_success ) { # scalar
        if ($parser) {
        ...
        if (defined($max_size) && $content_size > $max_size) {
            LWP::Debug::debug("Aborting because size limit of $max_size bytes exceeded");
        ...
        print OUT $$content;
        $content_size += length($$content);
        $entry->content_size($content_size); # update persistent size counter
        close(OUT);
        if (defined($max_size) && $content_size > $max_size) {
    ...
    elsif (ref($arg) eq 'CODE') { # read into callback
        if ($parser) {
            $parser->parse($$content) or undef($parser);
        }
        LWP::Debug::debug("[CODE] read " . length($$content) . " bytes");
        my $retval;
        eval { $retval = &$arg($$content, $response, $self, $entry); };
        if ($@) {
            chomp($@);
            $response->push_header('X-Died' => $@);
            $response->push_header("Client-Aborted", "die");
        } else {
            LWP::Debug::debug("return-code from Callback was '".(defined $retval ? "$retval'" : "[undef]'"));
            return $retval;   # pass return value from callback through to implementor class
        }
    } else {
        $response->code(&HTTP::Status::RC_INTERNAL_SERVER_ERROR);
...
sub receive_once {
    my ($self, $arg, $response, $content, $entry) = @_;
    my $no_content = '';
    my $retval = $self->receive($arg, $response,    \$content, $entry);  # read once
     $retval ||= $self->receive($arg, $response, \$no_content, $entry);  # or immediately simulate EOF
